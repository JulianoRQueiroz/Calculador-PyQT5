...
class MainWindow(qtw.QWidget):              # Janela Principal 
    ...
    def keypad(self):                       # Container Vazio
        ...
        btn_clear = qtw.QPushButton('Limpar', clicked = self.clear_calc)
        ...
        btn_3 = qtw.QPushButton('3', clicked = lambda: self.num_press('3'))    # A ordem no escopo não importa
    ...
    def num_press(self, key_number): # função de input dos numeros dos botões 
    ...
    def func_press(self, operator):  # função de input dos operadores dos botões 
        temp_string = ''.join(self.temp_nums)
...
mw = MainWindow()
app.setStyle(qtw.QStyleFactory.create('Fusão'))
app.exec_()